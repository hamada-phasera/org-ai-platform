...
export function DeliverableBar({ onCreate, busy, disabled }: Props) {
  return (
    <div className="mb-2 flex flex-wrap items-center gap-1.5">
      <span className="text-micro text-text-muted">この内容から成果物を作成:</span>
      {ITEMS.map((it) => (
        <button
          key={it.kind}
          type="button"
          ...
          className="flex items-center gap-1 rounded-full border border-border bg-elevated px-2.5 py-1 text-xs text-secondary transition-all hover:border-accent hover:text-accent disabled:opacity-50"
        >
