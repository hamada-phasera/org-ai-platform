...
-- Pre-aggregated daily rollup.  Keyed by (org_id, day, provider, model) so cost
-- stays model-aware (priced in the Go service) and the worker upsert is
-- idempotent.  Reading it for a 30-day window is ~0.05ms (index scan, ~5 rows).
CREATE TABLE IF NOT EXISTS usage_daily_rollup (
  ...
  p95_latency_ms integer  NOT NULL,   -- exact p95 for THIS (day,provider,model)
  PRIMARY KEY (org_id, day, provider, model)
);

-- Idempotent backfill / refresh.  The PHASE 4 worker runs this same upsert
-- incrementally (bounded to recent days).  Re-running it is safe — ON CONFLICT
-- makes it converge, never duplicate.
--
-- NOTE on p95: it is exact per (day, provider, model).  A window-wide p95 across
-- many days is NOT derivable from per-day p95s (percentiles don't compose); the
-- service computes window-total p95 from raw rows on demand.  See DESIGN.md.
INSERT INTO usage_daily_rollup (org_id, day, provider, model, calls, sum_tokens, sum_latency_ms, count_latency, p95_latency_ms)
SELECT
  "orgId", ("createdAt")::date, provider, model,
  count(*),
...
FROM "AILog"
GROUP BY "orgId", ("createdAt")::date, provider, model
ON CONFLICT (org_id, day, provider, model) DO UPDATE SET
...
-- The service already runs every read inside a READ ONLY transaction.  In prod,
-- additionally connect with a least-privilege role that can read AILog but only
-- write the rollup it owns:
--
