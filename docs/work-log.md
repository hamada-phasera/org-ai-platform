...
### 追記: 実 DB E2E スモーク検証 ✅（2026-07-16 後半セッション）
throwaway Postgres 16（ローカルクラスタ・pgvector 部分のみ text 代替 = RAG 対象外）+ スタブ AI Engine + gateway dist で新フロー一式を実際に叩いて検証:
- **マイグレーション**: `20260716000000_add_deal_and_task_scheduled_at` を含む全7本が実 DB に適用成功。
- **Deal (Prisma化)**: 作成(LEAD)→WON 遷移 OK、WON→LEAD は `INVALID_TRANSITION` 409 で正しく拒否。
- **提案書生成 (D1)**: POST → 202+QUEUED → n8n webhook 不達（意図的にダミー URL）→ **AI Engine フォールバック**が発火 → `DONE` + `output={proposal, templateId, model}`。
- **SNS 下書き (D1)**: POST → 202 → **PENDING_APPROVAL** + 整形済みドラフト（`#test`→`test` 正規化・validation 付き）。`/llm/chat` に json_mode=true / department=MARKETING / **user_email（松竹梅）が貫通**することをスタブ側ログで確認。
- **承認→予約→投稿準備 (D2 段階1)**: approve→APPROVED、schedule→`scheduledAt` カラム保存、`/n8n/sns-publish-prep` → `publishPrep{preview, queued:true, livePosted:false}` 付与・status は APPROVED のまま＝**実投稿なし**。
- **KPI アラート**: 失敗率 70% (7/10) をシード → `/n8n/kpi-alert-check` が FAILURE_RATE **HIGH** を検知し RiskEvent 1件作成。metrics svc 不達は `metricsUnavailableOrgs` として優雅に縮退。**再実行で重複作成 0（冪等）**。
- **認証**: cron 系 webhook は token 無しで 401。
