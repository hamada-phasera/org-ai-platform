...
- OAuth の `state` は有効期限 10 分の専用 HMAC 署名（ログイン JWT とは別鍵）。
  ログイン JWT と同じ鍵で署名すると、URL に載る state がそのまま API のベアラトークンとして
  通ってしまうため、鍵を用途で分けている（`OAUTH_STATE_SECRET`、未設定なら JWT_SECRET から派生）
- **接続先の組織はコールバックではなく「確認 API を叩いた本人のログインセッション」で決まる**。
  コールバックはトークンをどの組織にも紐づけずに一時保管し、フロントが `POST /api/oauth/google/confirm`
  を認証付きで呼んで初めて保存される。これにより「攻撃者が作ったリンクを踏ませて、
  被害者の Google を攻撃者の組織に繋ぐ」タイプの攻撃が成立しない
- コールバック後のリダイレクト先は `FRONTEND_URL` 固定（オープンリダイレクトなし）。
  `FRONTEND_URL` に `*` やカンマ区切りが入っていても、http(s) として妥当な先頭候補だけを採用する
- コールバックの URL には認可コードが載るため、このルートだけアクセスログを出さない
